...
def index():
    ...
    stats = {
        'most_categories' : 'Category',
        'total': 'Jumlah',
        'rev_table' : playstore.groupby(by=['Category', 'App']).sum(numeric_only=True)[['Reviews', 'Rating']].sort_values(by='Reviews', ascending=False).head(10).reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
